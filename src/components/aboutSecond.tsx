...
export default function AboutSecond() {
  ...
  useEffect(() => {
    ...
    if (!section || !leftImg || !rightImg || !tagline || !heading || !para) return;
    ...
    const initAnimation = () => {
      const ctx = gsap.context(() => {
        ScrollTrigger.getAll().forEach(trigger => {
          if (trigger.vars?.id === 'about-second-section-trigger') {
            trigger.kill();
          }
        });
        ...
        gsap.set([tagline, heading, para], { opacity: 0 });
        ...
        scrollTriggerInstance = ScrollTrigger.create({
          ...
          id: 'about-second-section-trigger',
          animation: tl,
          onRefresh: () => {
            gsap.set(leftImg, { opacity: 1, x: "-100vw", rotation: -8.53 });
            gsap.set(rightImg, { opacity: 1, x: "100vw", rotation: 8.53 });
    ...
    >
      <div className="w-full h-full">
        <div className="flex items-center justify-between gap-12 h-screen px-2">
          ...
          {/* Center Content */}
          <div className="flex-1 text-center relative min-w-0 flex flex-col items-center">
            <p ref={taglineRef} className='font-lora text-xl text-[#BB7F2557] mb-4'>Welcome to Evara</p>

            <h2 ref={headingRef} className="font-cinzel-decorative text-4xl text-[#00223F] mb-8 w-full">
              <span className="block">Where Every Love Story</span>
              <span className="block">Finds Its Magic</span>
            </h2>

            <div className="max-w-lg mx-auto">
              <p ref={paraRef} className="font-roboto text-sm text-[#BB7F25] leading-relaxed mb-12">
